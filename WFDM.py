...
from .LocalAttention-Invoking import LA
...
class GlobalAttention(nn.Module):
    ...
    def pad1(self, x, ps):
        ...
        _, _, H, W = x.size()
        padding = (0, ps - W % ps if W % ps != 0 else 0, 0, ps - H % ps if H % ps != 0 else 0)
...
class WFDM(nn.Module):
    ...
    def forward(self, x, imagename=None):
        # Perform wavelet transform and split the high-frequency components
        yL, yH = self.wt(x)
        y_HL = yH[0][:, :, 0, ::]
        y_LH = yH[0][:, :, 1, ::]
        y_HH = yH[0][:, :, 2, ::]
        ...
        # Set up a grid to display all 5 tensors side by side (including x)
        # fig, axes = plt.subplots(1, 5, figsize=(25, 5))

        # Function to plot a single tensor for grid display
        # def plot_in_grid(tensor, ax, title):
        #     tensor = tensor[0, 0, :, :].cpu().detach().numpy()  # Assuming single channel
        #     ax.imshow(tensor, cmap='viridis')  # You can change 'viridis' to any color map you prefer
        #     ax.set_title(title)
        #     ax.axis('off')
        #
        # # Plot the input x and the other tensors in the grid
        # plot_in_grid(x, axes[0], 'Input x')
        # plot_in_grid(yL, axes[1], 'yL')
        # plot_in_grid(yH, axes[2], 'yH')
        # plot_in_grid(glb, axes[3], 'glb')
        # plot_in_grid(local, axes[4], 'local')
        #
        # plt.show()
        ...
        def plot_in_grid(tensor, ax, title):
            ...
            tensor_mean = torch.mean(tensor, dim=1, keepdim=True)  # Shape: [batch_size, 1, H, W]
            tensor_mean = tensor_mean[0, 0, :, :].cpu().detach().numpy()  # Assuming single channel

            # Calculate the variance across spatial dimensions for each channel
            channel_variances = torch.var(tensor, dim=[2, 3])  # Shape: [batch_size, num_channels]
            most_feature_channel = torch.argmax(channel_variances, dim=1)  # Shape: [batch_size]

            # Select the most feature-rich channel
            tensor_feature = tensor[0, most_feature_channel[0], :, :].cpu().detach().numpy()  # Assuming single channel

            # Plot mean and most feature-rich channel side by side
            ax[0].imshow(tensor_mean, cmap='viridis')
            ax[0].set_title(f'{title} (Mean)')
            ax[0].axis('off')

            ax[1].imshow(tensor_feature, cmap='viridis')
            ax[1].set_title(f'{title} (Most Feature)')
            ax[1].axis('off')
